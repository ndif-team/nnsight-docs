...
.. raw:: html
    ...
    <script>
        ...
        function getDedicatedBadge(dedicated){

            if (!dedicated){
                return "";
            }

            let content = `<i class="fa-solid fa-thumbtack"></i>  Dedicated`
            let tooltip = "This model is guaranteed to be running until the scheduled end time."

            return getBadge(content, "info", bg=true, tooltip=tooltip, cls="electrolize")
        }
        ...
        function getScheduleBadges(schedule, deployment_level) {

            if (schedule === undefined || schedule === null) {
                return "";
            }

            let info_badge = getBadge(formatSchedule(schedule), "muted", bg=false)

            let schedule_badge = "";

            if (deployment_level !== "HOT") {
            

                let tooltip = "This model is scheduled to be a dedicated deployment for a duration specified by the deployment schedule.";
                let color = "info";
                let text = `<i class="fa-solid fa-clock"></i>  Scheduled`

                schedule_badge = getBadge(text, color, bg=true, tooltip=tooltip, cls="electrolize")
            }

            return `${info_badge}${schedule_badge}`
        }
                                        ...
                                        Object.entries(response.deployments)
                                            ...
                                            <div class="nn-deployment sd-card sd-sphinx-override sd-shadow-sm sd-card-hover" data-repo-id="${deployment.repo_id}" data-deployment-level="${deployment.deployment_level}" data-application-state="${deployment.application_state}" data-schedule="${deployment.schedule}">
                                                <div class="sd-card-body nn-card-body">
                                                    ...
                                                    <div class="nn-badges">
                                                        ${getScheduleBadges(deployment.schedule, deployment.deployment_level)}
                                                        ${getDedicatedBadge(deployment.dedicated)}
